...
def get_random_crop_old(height, width, scale, ratio, seed=None):
    area = height * width
    ...
    if second_seed is not None:
        ...
        for _ in range(10):
            ...
            if 0 < w2 <= width and 0 < h2 <= height:
                # i2 = random.randrange(0, height - h2 + 1, h2)
                # j2 = random.randrange(0, width - w2 + 1, w2)
                i2 = int(np.random.uniform(0, height - h2 + 1))
                j2 = int(np.random.uniform(0, width - w2 + 1))
                for _ in range(10):
                    if seed is not None:
                        np.random.seed(seed)
                    target_area2 = (area - target_area) #* np.random.uniform(scale[0], scale[1])
                    ...
                    if 0 < w <= width and 0 < h <= height:
                        if i2 - h + 1 > 0:
                            ...
                            return i, j, h, w
                            # j = int(np.random.uniform(0, j2))
                            # print("-----")
                            # print(width, height)
                            # print(j2, w2, i2, h2)
                            # print(j ,w, i, h)
                            # print(not (j+w < j2 or j > j2+w2 or i+h < i2 or i > i2 + h2))
                break
...
class LabelRandomResizedCropDecoder(rgb_image.RandomResizedCropRGBImageDecoder):#IntDecoder, ndarray.NDArrayDecoder):
    ...
    def generate_code(self) -> Callable:
        ...
        if second_seed is None:
            ...
            def decode(indices, my_storage, metadata, storage_state, counter):
                #def decode(indices, destination, metadata, storage_state, counter):
